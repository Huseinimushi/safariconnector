...
export default function OperatorBookingDetailPage() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        // get operator row (operators_view -> fallback operators)
        ...
        // load booking that belongs to this operator
        console.log("Loading booking", bookingId, "for operator", opRow.id);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (!data) {
        setErrorMsg("No booking was updated. Please check the ID/operator.");
        setSaving(false);
        return;
      }
        ...
        {loading ? (
          ...
          <>
              ...
              <div>
                ...
                >
                  ...
                  {booking.date_from || booking.date_to
                    ? `${formatDateShort(
                        booking.date_from
                      )} – ${formatDateShort(booking.date_to)}`
                    : "Not set"}
                </div>
              </div>
              ...
              >
                ...
                <span
                  style={{
                    ...
                    fontWeight: 600,
                    backgroundColor:
                      (booking.payment_status || "unpaid") === "paid_in_full"
                        ? "#ECFDF5"
                        : (booking.payment_status || "unpaid") === "deposit_paid"
                        ? "#EFF6FF"
                        : "#FEF3C7",
                    border:
                      (booking.payment_status || "unpaid") === "paid_in_full"
            ...
            >
              ...
              >
                Use this control to keep the booking and payment status in sync
                with your actual records. For example, when you receive a bank
                transfer, set payment status to <strong>Paid in full</strong>{" "}
