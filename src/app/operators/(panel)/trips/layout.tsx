  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      if (error) {
        ...
        if (alive) {
          setCompanyName(op2?.company_name ?? null);
          setLoading(false);
        }
        return;
      }
  ...
  return (
    <div style={{ minHeight: "100vh", display: "flex", background: BRAND.bg }}>
      ...
      >
        {NAV.map((item) => {
          ...
          return (
            ...
            >
              ...
              <div style={{ fontSize: 10, color: active ? "#FFF" : "#D1D5DB", fontWeight: active ? 700 : 500 }}>
                {item.label}
              </div>
            </Link>
          );
        ...
        >
          <div style={{ fontSize: 9, textTransform: "uppercase", letterSpacing: "0.16em", color: "rgba(226,232,240,0.9)", marginBottom: 4 }}>
            Your company
          </div>
          <div style={{ fontSize: 11, fontWeight: 700, color: "#F9FAFB", lineHeight: 1.2, wordBreak: "break-word" }}>
