 ...
 * We keep it conservative: if user mentions nights/overnight -> multi.
 */
...
/**
 * Hard normalize so Day trip NEVER becomes 7 days.
 * Even if AI returns wrong daysCount/days array, we enforce.
 */
function normalizeResult(
  ...
  const safe: ItineraryResult = {
    ...
    daysCount: Number((parsed as any)?.daysCount || 0),
    travelDate: (parsed as any)?.travelDate ?? null,
    budgetRange: String((parsed as any)?.budgetRange || "").trim(),
    style: String((parsed as any)?.style || "").trim(),
    groupType: String((parsed as any)?.groupType || "").trim(),
    experiences: Array.isArray((parsed as any)?.experiences) ? (parsed as any).experiences : [],
    days: Array.isArray((parsed as any)?.days) ? (parsed as any).days : [],
    includes: Array.isArray((parsed as any)?.includes) ? (parsed as any).includes : [],
    excludes: Array.isArray((parsed as any)?.excludes) ? (parsed as any).excludes : [],
  };
  ...
  if (!safe.title)
    safe.title =
      intentTripType === "day_trip"
        ? `Day trip: ${safe.destination}`
        : `Safari itinerary: ${safe.destination}`;
...
export default function AIGeneratorPage() {
  ...
  // lead capture minimal (only needed for PDF + operator followup)
  ...
  async function generate() {
    ...
    try {
      ...
      const parsed =
        safeJsonParse<ItineraryResult>(text) || extractJsonObject<ItineraryResult>(text);
  ...
  async function downloadPdf() {
    ...
    try {
      const res = await fetch("/api/itinerary/pdf", {
        ...
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          itinerary: result, // ✅ route now accepts this shape
  ...
  /**
   * FIXED: Send to operator via server route (service role),
   * avoids RLS & "table not found" schema cache issues from client insert.
   */
  async function sendToOperator() {
    ...
    try {
      const payload = {
        operator_id: "public", // TODO: replace when you add operator selection
        anon_id: anonId,
        user_id: user?.id ?? null,
        traveller_name: nm,
