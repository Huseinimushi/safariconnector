...
function normalizeResult(
  ...
  const safe: ItineraryResult = {
    ...
    activities: Array.isArray((parsed as any)?.activities) ? (parsed as any).activities : [],
  };
  ...
  safe.activities = safe.activities.map((x) => String(x || "").trim()).filter(Boolean).slice(0, 20);
  ...
  if (!safe.activities.length && safe.experiences.length) safe.activities = safe.experiences.slice(0, 8);
                    ...
                    {result.activities?.length > 0 && (
                      <>
                        ...
                        <div style={S.activityList}>
                          {result.activities.map((x) => (
                            <div key={x} style={S.activityItem}>
                              {x}
                            </div>
                          ))}
                        </div>
...
const S: Record<string, React.CSSProperties> = {
  ...
  activityList: { marginTop: 8, display: "flex", flexDirection: "column", gap: 8 },
  activityItem: { border: `1px solid ${BRAND.line}`, borderRadius: 12, padding: "10px 12px", background: "#fff", fontWeight: 850, color: BRAND.ink },
